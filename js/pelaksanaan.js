...
async function loadKaryawan() {
  ...
  try {
    console.log("Mencoba load karyawan...");
    ...
    console.log("Berhasil load", data.length, "karyawan");
...
/**
 * Tampilkan checklist sesuai jenis yang dipilih.
 * Sembunyikan seksi lain agar tidak ada data silang.
 */
...
function onToggleItem(e) {
  ...
  const labelEl = e.target.closest(".pel-toggle-wrap")
                           .querySelector(".pel-toggle-label");
...
function updateSkor(jenis) {
  ...
  // Pastikan nilai 0–100 (constraint database)
...
/** Kembalikan suffix class CSS berdasarkan nilai skor */
...
function validateEnableSubmit() {
  ...
  // Aktif hanya jika semua field wajib terisi & checklist sudah ditampilkan
  btn.disabled = !(tanggal && jenis && pemeriksa && currentJenis);
}
...
function bindEvents() {
  ...
  document.getElementById("jenis_inspeksi").addEventListener("change", (e) => {
    ...
    if (currentJenis) {
      ...
      document.getElementById("checklist-title").textContent =
        `Butir Pemeriksaan – ${currentJenis}`;
  ...
  // Validasi realtime saat field lain berubah
  ["tanggal", "pemeriksa_id"].forEach((id) => {
...
async function handleSubmit(e) {
  ...
  // Validasi client-side skor (sesuai constraint DB: 0–100)
  ...
  try {
    ...
    showToast(
      `✅ Inspeksi ${jenis_inspeksi} berhasil disimpan! Skor: ${skorAkhir}/100`,
      "success"
    );
...
function resetForm() {
  ...
  document.getElementById("skor-meter-fill").style.width = "0%";
  document.getElementById("skor-meter-fill").className   = "pel-skor-meter-fill";
...
async function loadHistory() {
  ...
  try {
    ...
    tbody.innerHTML = data.map((row) => {
      ...
      const namaStr = row.karyawan?.nama
      ...
      const catatanStr = row.catatan
        ? `<span title="${row.catatan}"
             style="display:inline-block;max-width:160px;
                    ...
                    white-space:nowrap;font-size:.8rem;color:#6b7280;">
             ${row.catatan}
           </span>`
    ...
    tbody.innerHTML = `
      <tr>
        <td colspan="6"
          style="color:#e02424;text-align:center;padding:16px;font-size:.85rem;">
...
/**
 * Tampilkan pesan toast di pojok kanan bawah.
